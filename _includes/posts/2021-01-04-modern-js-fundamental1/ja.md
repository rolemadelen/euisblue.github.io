...
`Node.js`などのサーバ勧業では`node my.js`のようにコマンドでと一緒に実行することができます。
...
⚠️ すごく簡単なコードの場合にはHTMLに`script`タグで挿入してもよろしいですが、直接挿入することは勧奨されないです。下の**外部スクリプト**パートを参照してください。

### 最新のマークアップ

- `type` 属性: <br>
    HTML4では`<script type='text/javascript'>`みたいにタイプを明示するのが必須でした。でもモダーンHTMLではその意味が全然変わって、JavaScriptモジュールに使用されます。

- `language` 属性: <br>
`<script language="javascirpt">`は「scriptにJavaScriptプログラムを挿入します」という意味でscriptの言語を表示するために使いました。いまはJavaScriptが基本系なので今は意味があまりない属性です。

- コメント(comments): <br>
    スクリプトの中に下記のようにコメントがついてるコードを向き合うことになるかも知れません。
    ```html
    <script type="text/javascript"><!--
    ...
    //---></script>
    ```
    このコードはブラウザが`script`タグを読めない場合を対応して、スクリプトが実行されるのを防止するために使いました。いまはほとんどのブラウザが`script`タグをサポートするので上のようなコードを見ると、非常に古いコードであると思ってもオッケーです。

### 外部スクリプト

JavaScriptプログラムを複数のファイルに分割できます。分割されたファイルは`<script src='test.js'>`みたいに参照えきるし、またはURLをプロパティで渡すこともできます。
...
ブラウザはダウンロードしたスクリプトをキャッシュ(cache)に保存するので性能に利点もあります。例えば複数のぺーじで同じスクリプトを使う場合ブラウザは毎回ダウンロードせず、キャッシュに保存されたスクリプトをロードして使います。
...
- 自動シミコロン挿入(automatic semicolon insertion)
    ```jsx
    alert('hello')
    alert('world')
    ```
    → JavaScriptは改行を'暗黙'のセミコロンと解釈します。しかし”常に”ではありません。なので文の最後には忘れずセミコロンをつけてください。
...
- 2009年ECMAScript5(ES5)が登場する前までは、JSに互換性として問題はなかったです。
- ES5からの新しい機能がJSの本来の機能の一部を変わったので下位互換性の問題が発生する可能性があります。なので普通のときはES5の変更事項を適用しないようにしました。
- スクリプトの最上段に`"use strict"`を入れると、モダンなモーとでJavaScriptが実行されます。ES5の文法が適用されます。
