...
한쪽 끝에서만 자료의 추가/제거가 가능한 **LIFO** (Last-In First-Out) 또는 **FILO** (First-In Last-out) 형식의 선형(linear) 자료구조.

<div class="divider"></div>
## 연산 <a id="op"></a>
스택(Stack)에서의 자료 추가/제거는 전부 **스택의 끝**에서 연산이 이루어지며 가장 최근에 추가된 자료가 스택의 끝에 위치한다.
 - **push**: 스택에 데이터를 추가한다
 - **pop**: 스택에서 최근 항목을 삭제한다
 - **top**: 스택의 최근 항목을 반환한다
 - **isEmpty**: 스택이 비어있는지 확인한다
 - **size**: 스택에 들어 있는 항목의 총 개수를 반환한다
