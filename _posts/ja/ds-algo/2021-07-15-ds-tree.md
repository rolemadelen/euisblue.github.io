...
親がないツリーの最初のノードを**根 (root)** ノードといいます。 根ノード一つのツリーに一つし存在しません。各ノードは０または０以上の**子ノード (child node)**を持ちます。最大二つの子ノードを持つ木構造を**2分木 (Binary Tree)**といいます。２分木については次の記事でもっと詳しく話します。
...
各用語のもっと詳しい説明と例えは[こちらへ](https://www.gatevidyalay.com/tree-data-structure-tree-terminology/){:target="_blank"}。
...
C及びC++ような言語では直接割り当てたメモリを解放する必要があります。

その一つの方法としてはrecursionを使用することです。次回に学ぶpreorderという方法で、木の三つの巡回方法中一つです。
